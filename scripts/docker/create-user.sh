...
if ! getent group transfer-group > /dev/null 2>&1; then
    addgroup -g "$PGID" transfer-group
fi

# Check if a user with PUID exists; if not, create it
if ! id -u transfer > /dev/null 2>&1; then
    if ! getent passwd "$PUID" > /dev/null 2>&1; then
        adduser -u "$PUID" -G transfer-group transfer > /dev/null 2>&1
    else
