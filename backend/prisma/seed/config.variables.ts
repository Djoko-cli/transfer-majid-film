...
// Pure data: no Prisma client, no database access, no side effect beyond
// reading the legal .md files and generating jwtSecret's random bytes at
// import time. Split out of config.seed.ts (which calls seedConfigVariables()
// against the live database at module scope) so config.seed.spec.ts can
// import configVariables without seeding the dev database.
...
//
// `__dirname` itself is only defined under ts-node/dist's CommonJS load
// (the real seeding paths, dev and prod). config.seed.spec.ts pulls this
// module in through node's native ESM loader instead, where `__dirname`
// doesn't exist - falling back to cwd() there is harmless: it can only
// make legalContentDir resolve to a directory without the .md files, and
// readLegalMarkdown() below already tolerates that by returning "" rather
// than crashing (its own comment explains why), and the invariant test
// this module exists for never inspects imprint/terms/privacyPolicy text.
const moduleDir = typeof __dirname !== "undefined" ? __dirname : process.cwd();
const legalContentDir = fs.existsSync(path.join(moduleDir, "../../../legal"))
  ? path.join(moduleDir, "../../../legal")
  : path.join(moduleDir, "../../..");
