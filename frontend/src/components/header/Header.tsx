...
const useStyles = createStyles((theme) => {
  ...
  return {
    ...
    mobileMenuReveal: {
      ...
      // Safari, only this nested one didn't. `top: HEADER_HEIGHT`
      // replaces the old `top: 100%`, which relied on being positioned
      // relative to the header's own box; as a sibling this now needs
      // the real pixel offset instead.
      //
      // The reveal itself is a clip-path transition on this box (see the
      // inline style at its JSX), not the transform+opacity this used to
      // animate — confirmed directly, frame-by-frame from a real-device
      // recording, that Safari renders the Paper below as flat black
      // *specifically during* the open/close transition (correctly
      // frosted again the instant it settles): backdrop-filter combined
      // with an animating transform/opacity, on the filtered element
      // itself or an ancestor, is a separate, well-documented WebKit
      // rendering bug — a willChange hint (tried first) reduces how
      // often it fires but doesn't eliminate it. clip-path sidesteps the
      // whole category: nothing in Paper's ancestor chain, or Paper
      // itself, ever animates transform or opacity anymore, only how
  ...
  return (
    <>
      ...
      </MantineHeader>
      {
        ...
        // transition). scaleY(0)-equivalent (inset(0 0 100% 0)) rather
